...
class SaltAndPepperNoise:
    ...
    def take_bias_frames(self, cam_handle, cam_id, num_frames=100):
        ...
        for frame_num in range(num_frames):
            ...
            if success != cam.QHYCCD_SUCCESS:
                print(f'Failed to set resolution for frame {frame_num}:', success)
                continue

            # Set exposure to 0 for bias frames
            success = cam.so.SetQHYCCDParam(cam_handle, cam.CONTROL_EXPOSURE, c_double(0))
            if success != cam.QHYCCD_SUCCESS:
                print(f'Failed to set exposure for frame {frame_num}:', success)
                continue

            success = cam.so.ExpQHYCCDSingleFrame(cam_handle)
            if success != cam.QHYCCD_SUCCESS:
                print(f'Exposure failed for frame {frame_num}:', success)
                continue
            ...
            success = cam.so.GetQHYCCDSingleFrame(cam_handle,
                                                  ...
                                                  prev_img_data)
            if success == cam.QHYCCD_SUCCESS:
                print(f'Frame {frame_num} captured successfully!')
                image = np.ctypeslib.as_array(prev_img_data).reshape(i_h, i_w)
                bias_frames.append(image)
                self.save_fits_image(image, f'bias_frame_{frame_num}.fits')
            else:
                print(f'Failed to capture frame {frame_num}', success)
    ...
    def calculate_salt_and_pepper_noise(self, bias_frames):
        ...
        # Check for any frames with pixel values starting at 0
        for frame_idx, frame in enumerate(bias_frames):
            if np.any(frame == 0):
                print(f"Frame {frame_idx} has pixels with value 0")
...
def setup_camera(cam_id, read_mode=1, target_temp=-5.0, gain=56):
    ...
    if success != cam.QHYCCD_SUCCESS:
        print('Failed to set read mode:', success)
    ...
    if success != cam.QHYCCD_SUCCESS:
        print('Failed to set stream mode:', success)
    
    success = cam.so.InitQHYCCD(cam_handle)
    if success != cam.QHYCCD_SUCCESS:
        print('Failed to initialize camera:', success)

    success = cam.so.SetQHYCCDBitsMode(cam_handle, c_uint32(cam.bit_depth_16))
    if success != cam.QHYCCD_SUCCESS:
        print('Failed to set bits mode:', success)
    ...
    if success != cam.QHYCCD_SUCCESS:
        print('Failed to get chip info:', success)
...
def perform_salt_and_pepper_noise_test(cam_id, read_mode=1, target_temp=-5.0, gain=56, num_bias_frames=100):
...
def pnp_in(cam_id):
    ...
    perform_salt_and_pepper_noise_test(cam_id, read_mode=1, target_temp=-5.0, gain=56, num_bias_frames=100)
